...
def verify(call):
    ...
    try:
        ...
        bot.answer_callback_query(call.id, "⚠️ Gagal mengecek channel! Pastikan Bot sudah ADMIN.", show_alert=True)
...
def proses_setor(message):
    ...
    try:
        ...
        if not validate_email(email):
            bot.reply_to(message, "<blockquote>❌ <b>FORMAT SALAH:</b> Format email tidak valid.</blockquote>", parse_mode='HTML')
            return
...
✅ <i>IMAP LIVE & Prefix Match!</i></blockquote>
        ...
        bot.reply_to(message, "<blockquote>❌ <b>FORMAT SALAH!</b> Gunakan format: <code>email|password</code></blockquote>", parse_mode='HTML')
...
💳 <b>Sisa Saldo:</b> <code>{format_rupiah(user[6])}</code></blockquote>
        ...
        bot.reply_to(message, "request WD Gagal. Gunakan format: <code>nomor|jumlah</code>", parse_mode='HTML')

# ========== ADMIN ACTIONS (FIXED CALLBACK) ==========
...
def admin_approve_gmail(call):
    ...
    # Parsing callback format: "approve_gmail_ID"
    data_parts = call.data.split('_')
    action = data_parts[0]  # 'approve' atau 'reject'
    gmail_id = int(data_parts[2])

    status = 'verified' if action == 'approve' else 'rejected'
    gmail = db.approve_gmail(gmail_id, call.from_user.id, status)
    
    if not gmail:
        bot.answer_callback_query(call.id, "Data Gmail tidak ditemukan di DB.", show_alert=True)
        return
    ...
    if action == 'approve':
        ...
        bot.answer_callback_query(call.id, "✅ Gmail berhasil di-ACC!")
    else:
        msg = f"<blockquote>❌ <b>ɢᴍᴀɪʟ ᴅɪᴛᴏʟᴀᴋ</b>\n📧 Email: <code>{gmail[1]}</code></blockquote>"
        bot.answer_callback_query(call.id, "❌ Gmail telah ditolak.")
...
# ========== ADMIN PANEL (FIXED COMPLETE HANDLERS) ==========
...
@bot.callback_query_handler(func=lambda call: call.data == 'admin_gmail')
def admin_gmail_pending(call):
    if call.from_user.id not in ADMIN_ID:
        return
    pending = db.get_pending_gmails()
    bot.answer_callback_query(call.id)
    if not pending:
        text = "<blockquote><b>📥 GMAIL PENDING</b>\n\n<i>Tidak ada antrean Gmail pending saat ini.</i></blockquote>"
    else:
        text = f"<blockquote><b>📥 GMAIL PENDING ({len(pending)})</b>\n\n"
        for row in pending[:10]: # Tampilkan max 10 item
            text += f"• ID: <code>{row[0]}</code> | <code>{row[1]}</code> | User: <code>{row[3]}</code>\n"
        text += "</blockquote>"
    bot.send_message(call.message.chat.id, text, reply_markup=generate_back_markup(), parse_mode='HTML')

@bot.callback_query_handler(func=lambda call: call.data == 'admin_wd')
def admin_wd_pending(call):
    if call.from_user.id not in ADMIN_ID:
        return
    pending = db.get_pending_withdraws()
    bot.answer_callback_query(call.id)
    if not pending:
        text = "<blockquote><b>💸 WD PENDING</b>\n\n<i>Tidak ada antrean penarikan DANA saat ini.</i></blockquote>"
    else:
        text = f"<blockquote><b>💸 WD PENDING ({len(pending)})</b>\n\n"
        for row in pending[:10]:
            text += f"• ID WD: <code>{row[0]}</code> | User: <code>{row[1]}</code> | Rp <code>{row[2]:,}</code> | DANA: <code>{row[3]}</code>\n"
        text += "</blockquote>"
    bot.send_message(call.message.chat.id, text, reply_markup=generate_back_markup(), parse_mode='HTML')
...
def admin_stats(call):
    if call.from_user.id not in ADMIN_ID:
        return
    total_users, total_setor, total_balance = db.get_stats()
    bot.answer_callback_query(call.id)
    text = f"""
<blockquote><b>📊 ꜱᴛᴀᴛɪꜱᴛɪᴋ ʙᴏᴛ</b>

👥 <b>Total User:</b> <code>{total_users} User</code>
📦 <b>Total Akun Tersetor:</b> <code>{total_setor} Akun</code>
💳 <b>Total Saldo User:</b> <code>{format_rupiah(total_balance)}</code></blockquote>
    """
    bot.send_message(call.message.chat.id, text, reply_markup=generate_back_markup(), parse_mode='HTML')
...
if __name__ == '__main__':
    print("🚀 JOB GMAIL PRO BOT FULL ONLINE & FIXED TOTAL...")
    while True:
        try:
